...
def run():
    ...
    # Model Code Block
    word_idx_rev, word_id_mapping, word_embedding, pos_embedding = load_w2v_BERT_ext(FLAGS.embedding_dim,
    ...
    x = tf.placeholder(tf.int32, [None, 2, FLAGS.max_sen_len], name='x')
    sen_len = tf.placeholder(tf.int32, [None, 2], name='sen_len')
    keep_prob1 = tf.placeholder(tf.float32, name='keep_prob1')
    keep_prob2 = tf.placeholder(tf.float32, name='keep_prob2')
    distance = tf.placeholder(tf.int32, [None], name='distance')
    ...
    # 保存模型
    tf.add_to_collection('pred_pair', pred_pair)
    ...
    with tf.Session(config=tf_config) as sess:
        ...
        for fold in range(1, 11):
            ...
            tr_pair_id_all, tr_pair_id, tr_y, tr_x, tr_sen_len, tr_distance = load_data_2nd_step(
                save_dir + train_file_name, word_id_mapping, max_sen_len=FLAGS.max_sen_len)
            te_pair_id_all, te_pair_id, te_y, te_x, te_sen_len, te_distance = load_data_2nd_step(
            ...
            for i in xrange(FLAGS.training_iter):
                ...
                # train
                for train, _ in get_batch_data(tr_x, tr_sen_len, FLAGS.keep_prob1, FLAGS.keep_prob2, tr_distance, tr_y,
                                               FLAGS.batch_size):
                ...
                loss, pred_y, true_y, acc = sess.run([loss_op, pred_y_op, true_y_op, acc_op],
                ...
                print
                'pair filter keep rate: {}'.format(keep_rate)
            ...
            # 保存模型
            saver.save(sess, 'save/pair/model_P_cause_BERT_ext_pair.ckpt', global_step=fold)  #
            # 保存模型
...
def main(_):
    ...
    for scope_name in ['P_cause_BERT_ext']:
        FLAGS.scope = scope_name
        run()
