...
def run():
    ...
    y_cause = tf.placeholder(tf.float32, [None, FLAGS.max_doc_len, FLAGS.n_class], name='y_cause')
    placeholders = [x, sen_len, doc_len, keep_prob1, keep_prob2, y_position, y_cause]

    pred_pos, pred_cause, reg = build_model(word_embedding, x, sen_len, doc_len, keep_prob1, keep_prob2, y_position,
                                            y_cause)
    ...
    # 保存模型
    tf.add_to_collection('pred', pred_pos)
    tf.add_to_collection('pred', pred_cause)
    with tf.Session(config=tf_config) as sess:
        ...
        for fold in range(1, 11):
            ...
            # 保存模型
            saver.save(sess, 'save/model_P_emotion_BERT_ext.ckpt', global_step=fold)  #
                ...
                for i in range(len(doc_id)):
                    ...
                    for j in range(doc_len[i]):
                        ...
                        for k in range(sen_len[i][j]):
                            clause = clause + word_idx_rev[str(x[i][j][k])] + ' '
                        g.write(str(j + 1) + ', ' + str(pred_y_pos[i][j]) + ', ' + str(
                            pred_y_cause[i][j]) + ', ' + clause + '\n')
                print
                'write {} done'.format(file_name)
...
def main(_):
    ...
    FLAGS.scope = 'P_emotion_BERT_ext'
    run()
