...
def load_w2v_BERT(embedding_dim, embedding_dim_pos):
    print('\nload embedding_BERT...')

    # Load
    fileName1 = "./BERT_RES/word_idx.csv"
    with open(fileName1, "rb") as csv_file:
        reader = csv.reader(csv_file)
        word_idx = dict(reader)

    fileName2 = "./BERT_RES/word_idx_rev.csv"
    with open(fileName2, "rb") as csv_file:
        reader = csv.reader(csv_file)
        word_idx_rev = dict(reader)

    word_embedding = np.loadtxt('./BERT_RES/word_embedding.txt')

    embedding = [list(np.zeros(embedding_dim))]

    for item in word_embedding:
        vec = item.tolist()
        embedding.append(vec)
...
def load_w2v_BERT_ext(embedding_dim, embedding_dim_pos):
    print('\nload embedding_BERT...')
    ...
    word_embedding = np.loadtxt('./BERT_RES/wmm-ext/word_embedding.txt')
...
def prf_2nd_step(pair_id_all, pair_id, pred_y, fold = 0, save_dir = './pair_res/'):
    pair_id_filtered = []
    ...
    def write_log():
        ...
        g = open(save_dir+'pair_res_Ind_fold{}.txt'.format(fold), 'w')
